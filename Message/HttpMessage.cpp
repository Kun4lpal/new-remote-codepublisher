...
HttpMessage HttpMessage::parseMessage(std::string& src)
{
	std::stringstream ss(src);
	std::string io;
	HttpMessage msg;
	if (!src.empty()) {
		while (std::getline(ss, io, '\n')) {
			Attribute attrib;
			std::string cpy = io;
			attrib = msg.parseAttribute(cpy);
...
std::vector<std::string> HttpMessage::getTypeVectr()
{
	return typeVector;
}
...
std::string HttpMessage::patternsString() const
{
	std::string patternsString = typeVector[0];
	for (int i = 1; i < typeVector.size(); i++)
	{
		patternsString += (" " + typeVector[i]);
	}
...
void HttpMessage::addPattern(std::string pattern)
{
	typeVector.push_back(pattern);
}

//----< parse pattern string >--------

void HttpMessage::parsePatternString(std::string patternsString)
{
	while (patternsString.find(" ") != std::string::npos)
	{
		int pos = patternsString.find(" ");
		std::string temp = patternsString.substr(0, pos);
		typeVector.push_back(temp);
		patternsString = patternsString.substr(pos + 1);
	}
	typeVector.push_back(patternsString);
}
