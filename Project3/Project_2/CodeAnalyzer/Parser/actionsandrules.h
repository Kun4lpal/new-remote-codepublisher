...
namespace CodeAnalysis
{
	...
	{
	public:
		using Rslt = StaticLogger<0>;  // use for application results
		using Demo = StaticLogger<1>;  // use for demonstrations of processing
		using Dbug = StaticLogger<2>;  // use for debug output
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			else {if (typeIndex < pTc->length() && 1){
	...
	*/
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			declNode.pTc = pTc->clone(); declNode.path = p_Repos->currentPath();
